...
class StandardPatch {
    ...
    validatePatch(file) {
        ...
        if(status === "on") {
            ...
            return '"' + this.name + '" 既不是开启也不是关闭状态! 您是否拥有正确的文件?';
        }
    }
...
var loadPatch = function(_this, self, patcher) {
    ...
    self.successDiv.innerHTML = " 已加载" + successStr;
};
...
class PatchContainer {
    ...
    createUI() {
        ...
        label.innerHTML = "<strong>选择文件</strong> 或将文件拖放至此";
        ...
        container.appendChild(createElementClass('h5', null, '支持的版本:'));
        container.appendChild(createElementClass('h5', null, '点击名称预览补丁项'));
    ...
    loadFile(file) {
        ...
        reader.onload = function (e) {
            ...
            if (!found) {
                ...
                self.errorDiv.innerHTML = "无法100%匹配补丁项";
            }
        };
