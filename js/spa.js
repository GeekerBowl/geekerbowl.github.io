        ...
        if (pages[pageId]) {
			...
			if (pageId === 'fortune') {
			  ...
			  function displaySong(song) {
				...
				albumCover.src = `https://oss.am-all.com.cn/asset/img/main/music/${song.image}?t=${timestamp}`;
				
				// 添加错误处理
				albumCover.onerror = function() {
				  console.error('封面加载失败:', song.image);
				  albumCover.style.display = 'none';
				  placeholder.style.display = 'flex';
				  placeholder.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>封面加载失败</span>';
				};
